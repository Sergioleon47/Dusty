...
function agentExec(name, inp){
  ...
  switch(name){
    ...
    case 'delete_job': {
      ...
      const {pruned, restocked} = deleteJob(j);
      saveState(); logActivity('job_deleted', j.client); render();
      return ok(Object.assign({deleted: j.client, amount: j.price, date: j.date}, pruned>0 ? {upcoming_removed: pruned} : {}, restocked>0 ? {stock_restored: restocked} : {}));
    }
    case 'update_asset': {
      const a = agentFind(bizProfile.assets||[], inp.asset, x=>x.name); if(!a) return fail('asset not found');
      const ch = {};
